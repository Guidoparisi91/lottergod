...
func _ready():
	%ResetButton.pressed.connect(_reset_rotations)
...
func _input(event: InputEvent):
	...
	if event is InputEventMouseMotion \
		and event.button_mask & 2:
			# Sin modelo cargado no hay nada que rotar. El plugin original no
			# chequeaba esto y spameaba errores al mover el mouse en vacío.
			if models.is_empty() or current_model >= models.size():
				return
			var model: Node3D = models[current_model]["node"]
			...
			if not shift:
				...
				model.rotate_object_local(Vector3(0, 0, 1), rot_z)
